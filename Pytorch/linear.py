...
# 기존에는 0.0001이었으나, 학습률이 너무 높아서 최적의 가중치로 수렴하지 못하고 발산하는 형태가 됨
# 따라서 학습률을 조정하여 비용함수가 최소가 되는 지점으로 수렴하도록 함
lr = 0.00001

# 예측 함수 정의
# train함수 내부에서 전역변수 w,b의 값이 0에서 바뀌지 않는 것을 파악함
# 따라서 w,b를 파라미터로 넘겨주어 y_pred 값이 제대로 바뀌게 함
def predict(x,w,b):
    y_pred = w*x + b
    return y_pred
...
my_height = 159
# train함수를 호출 한 후 리턴 받은 w,b 변수를 파라미터로 넘겨주어 predict값을 잘 받아 오게 함
